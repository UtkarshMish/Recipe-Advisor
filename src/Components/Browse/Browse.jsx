...
class Browse extends Component {
	...
	async componentDidMount() {
		...
		currentPage = this.props.match.params.id && this.props.match.params.id > 0 ? parseInt(this.props.match.params.id) : currentPage;
	...
	handlePageChange = async (page) => {
		let { currentPage, cuisines, pageSize, startValue, searchQuery } = this.state;
		if (page > 0) {
			currentPage = page;
			startValue = page;
			this.loading();
			...
			if (cuisines.length > 0 && cuisines[cuisines.length - 1].totalSize != null) {
				const size = cuisines.pop();
	...
	render() {
		const { cuisines, isLoading, pageSize, currentPage, startValue, searchQuery, failed, isSearching } = this.state;
